...
export const downloadTicketPDF = async (req, res) => {
    try {
        ...
        const event = ticket.eventId;
        if (!event) return res.status(404).json({ message: "Event not found for this ticket" });

        const ticketType = event.ticketTypes.id(ticket.ticketTypeId);
        if (!ticketType) {
            // Fallback if ticket type was deleted or id mismatch
            const ticketWithDetails = {
                ...ticket.toObject(),
                ticketType: { name: "General Admission" } // Fallback name
            };
            const pdfBuffer = await generateTicketPDF(ticketWithDetails, event);
            res.set({
...
// Organizer: Generate ticket for a guest
export const generateGuestTicket = async (req, res) => {
    try {
        const { eventId, ticketTypeId, guestName, guestEmail } = req.body;
        ...
        if (event.organizerId.toString() !== req.user._id.toString()) {
            return res.status(403).json({ message: "Only the organizer can generate guest tickets" });
        }

        const ticketType = event.ticketTypes.id(ticketTypeId);
        if (!ticketType) return res.status(400).json({ message: "Invalid ticket type" });

        if (ticketType.quantity - ticketType.sold < 1) {
            return res.status(400).json({ message: "No tickets available for this type" });
        }

        // Create dummy order for tracking
        const order = new Order({
            eventId,
            buyerId: req.user._id, // Generated by organizer
            guestDetails: { name: guestName, email: guestEmail },
            items: [{ ticketTypeId, quantity: 1, price: 0 }],
            totalAmount: 0,
            status: "comp", // Complimentary
            paymentDetails: { paymentMethod: "organizer_comp", paymentDate: new Date() }
        });
        await order.save();
        ...
        // Notify guest if email provided
        if (guestEmail) {
            const html = `
                <h1>Your Invitation to ${event.title}</h1>
                <p>Hi ${guestName}, the organizer has generated a ticket for you.</p>
                <p>Ticket ID: ${uniqueId}</p>
                <p><a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/tickets/${uniqueId}">View Your Ticket</a></p>
            `;
            await sendEmail(guestEmail, `Your Invite: ${event.title}`, html).catch(console.error);
        }

        res.status(201).json({ message: "Guest ticket generated successfully", ticket });
    } catch (error) {
        res.status(500).json({ message: "Error generating guest ticket", error: error.message });
    }
};
